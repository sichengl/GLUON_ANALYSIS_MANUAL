...
with h5py.File(pt2_path, "r") as f:
    pt2_forward = f["pt2_forward"][:, :, :, :, :, :, 0]
    pt2_backward = f["pt2_backward"][:, :, :, :, :, :, 0]
    pf_list = f["pf_list"][:]
gamma_sign = np.array([1, -1, 1])
C_full = 0.5 * (pt2_forward.mean(axis=5)
                + gamma_sign[None, None, None, :, None, None, None] * gamma_sign[None, None, None, None, :, None, None] * pt2_backward.mean(axis=5))
...
sign = np.sign(np.trace(C.mean(axis=0)[:, :, 0, t0]).real)
C = sign * C

C_jk = (C.sum(axis=0) - C) / (n_cfg - 1)
C_all = np.concatenate([C_jk, C.mean(axis=0)[None]], axis=0)
n_samp = n_cfg + 1

t_list = np.arange(t0 + 1, tsep_max)
lam = np.full((n_samp, n_pf, n_op, len(t_list)), np.nan)
vec = np.full((n_samp, n_pf, n_op, len(t_list), n_op), np.nan, "<c16")
for isamp in range(n_samp):
    for ipf in range(n_pf):
        try:
            L = np.linalg.cholesky(C_all[isamp, :, :, ipf, t0])
        except np.linalg.LinAlgError:
            continue
        L_inv = np.linalg.inv(L)
        for it, t in enumerate(t_list):
            M = L_inv @ C_all[isamp, :, :, ipf, t] @ L_inv.conj().T
            w, u = np.linalg.eigh(M)
            v = L_inv.conj().T @ u
            order = np.argsort(w)[::-1]
            lam[isamp, ipf, :, it] = w[order]
            vec[isamp, ipf, :, it] = v[:, order].T
with np.errstate(invalid="ignore", divide="ignore"):
    E_eff = np.log(lam[..., :-1] / lam[..., 1:])
    C_ref = C_all[:, 4, 4]
    E_eff_ref = np.log(C_ref[..., :-1] / C_ref[..., 1:]).real
...
with h5py.File(out_path, "w") as f:
    ...
    f.attrs["v"] = "v[samp, pf, n, t, op]: eigenvector of state n at (t, t0), normalized to v^dag C(t0) v = 1"
    f.attrs["average"] = "0.5 * (forward + s_a s_b backward), s = (+1, -1, +1) for (G5, G45, G35)"
