...
OUT_DIR = SCRIPT_DIR / "gpd_3pt_forward_opp"

cfg_skip = [4314]
cfg_list = [204, 210, 216]
...
tgf_list = np.array([20], dtype=np.int64)
...
pt3_output = str(OUT_DIR / "3pt_tgf{tgf}_pz{pz}_q{qx}_{qy}_{qz}.h5")
...
def make_source_phase(cfg):
    ...
    q_dot_x = (
        x[:, None, None, None] * q_list[None, None, None, :, 0]
        + y[None, :, None, None] * q_list[None, None, None, :, 1]
        + z[None, None, :, None] * q_list[None, None, None, :, 2]
    )
...
def run_one_cfg(cfg):
    ...
    with h5py.File(ff_input.format(cfg=cfg), "r") as f:
        if not np.array_equal(q_list, f["asymmetric_qlist"][:]):
            raise ValueError(f"asymmetric q list mismatch in cfg {cfg}")
        w_list = f["wilson_line_list"][:]
        # [munu, rhosig, tgf, z_WL, q, t]
        FF_raw = f["asymmetric_corr"][0, ...]

    # [lorentz, tgf, z_WL, q, t]
    FF_lorentz = np.stack([FF_raw[i, j, tgf_list] for i, j in lorentz_list])
    if vev_subtract:
        FF_lorentz = FF_lorentz - FF_lorentz.mean(axis=-1, keepdims=True)
    # [op, tgf, z_WL, q, t]
    FF_op = np.stack([
        ...
        for i_pz, pf in enumerate(pf_list):
            for i_q, q in enumerate(q_list):
                ...
                out = pt3_output.format(
                    tgf=tgf, pz=pf[2], qx=q[0], qy=q[1], qz=q[2])
                with h5py.File(out, "w") as f:
                    f.create_dataset("pt2FF", data=pt2FF)
                    f.create_dataset("cfg_list", data=np.array(cfg_list))
                    f.create_dataset("w_list", data=w_list)
                    f.create_dataset("operator_names", data=op_names)
                    f.attrs["dim_pt2FF"] = "cfg,op,z_WL,tsep,tins"
                    f.attrs["tgf"] = tgf
                    f.attrs["pz"] = int(pf[2])
                    f.attrs["q"] = q
                    f.attrs["q_phase_sign"] = q_phase_sign
                    f.attrs["vev_subtract"] = vev_subtract
