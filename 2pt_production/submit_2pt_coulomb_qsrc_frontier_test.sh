...
#SBATCH -J pt2_coulomb_qsrc_test
#SBATCH -o ./logs/pt2_coulomb_qsrc_frontier_test_%j.out
#SBATCH -e ./logs/pt2_coulomb_qsrc_frontier_test_%j.err

# one-configuration debug test on one Frontier node, 8 GCDs, grid [1,1,1,8] in the python script, spatial sources contracted with q_list
# runs one full configuration (about 56 min) including the save: check SAVING SECTION in the log and the two h5 files
cd $SLURM_SUBMIT_DIR
mkdir -p logs wrappers
SCRIPT=2pt_coulomb_boosted_qsrc_64srcs_frontier.py
...
# default cfg 234 (ICFG=5): its q phases are not all 1, unlike cfg 204, so the phase is testable (override with: ICFG=0 sbatch submit_2pt_coulomb_qsrc_frontier_test.sh)
ICFG=${ICFG:-5}
