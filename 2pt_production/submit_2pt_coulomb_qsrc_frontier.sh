...
#SBATCH -o ./logs/pt2_coulomb_qsrc_frontier_%j.out
#SBATCH -e ./logs/pt2_coulomb_qsrc_frontier_%j.err

# 2pt (pion + proton), Coulomb-gauge boosted smearing on Frontier, spatial sources contracted with q_list: 40 configurations, one per node, 8 GCDs each,
...
cd $SLURM_SUBMIT_DIR
mkdir -p logs/coulomb_qsrc wrappers
...
for ICFG in $(seq 0 5 95) $(seq 100 5 195); do
    ...
    srun -u --exclusive -N 1 -n 8 --kill-on-bad-exit=1 \
        --output="./logs/coulomb_qsrc/cfg${CFG}_%j.out" --error="./logs/coulomb_qsrc/cfg${CFG}_%j.out" \
