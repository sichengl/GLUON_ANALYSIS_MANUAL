...
Ls = 32
Lt = 96
n = 20  #number of configs to measure, starting from 204, with step size 6
mom_min = 0
mom_max = 6
smear_steps = 40
...
cfg_stride = 2
measurement_list = cfg_list[::cfg_stride][:n]     # cfg_list[0], cfg_list[2], cfg_list[4], ... : 204, 216, 228, ...
measurement_list = cfg_list[0:n]
pion_45     = cp.zeros((t_src.shape[1], x_src.shape[1], y_src.shape[1], z_src.shape[1], len(momentum_list), latt_info.Lt), "<c16")   # (n, nt, nx, ny, nz, nmom, Lt)
pion_5      = cp.zeros((t_src.shape[1], x_src.shape[1], y_src.shape[1], z_src.shape[1], len(momentum_list), latt_info.Lt), "<c16")   # (n, nt, nx, ny, nz, nmom, Lt)
proton_45_p = cp.zeros((t_src.shape[1], x_src.shape[1], y_src.shape[1], z_src.shape[1], len(momentum_list), latt_info.Lt), "<c16")   # (1+g4)/2
proton_45_m = cp.zeros((t_src.shape[1], x_src.shape[1], y_src.shape[1], z_src.shape[1], len(momentum_list), latt_info.Lt), "<c16")   # (1-g4)/2
proton_5_p  = cp.zeros((t_src.shape[1], x_src.shape[1], y_src.shape[1], z_src.shape[1], len(momentum_list), latt_info.Lt), "<c16")
proton_5_m  = cp.zeros((t_src.shape[1], x_src.shape[1], y_src.shape[1], z_src.shape[1], len(momentum_list), latt_info.Lt), "<c16")
for i_cfg, cfg in tqdm(enumerate(measurement_list),desc=f"Processing cfgs"):
        ...
        for x_idx, x0 in enumerate(x_src[i_cfg]):
            for y_idx, y0 in enumerate(y_src[i_cfg]):
                for z_idx, z0 in enumerate(z_src[i_cfg]):
                    ...
                    deviceSynchronize()
                    s = perf_counter()
                    pion_45[t_idx,x_idx,y_idx,z_idx] += contract(
                    ...
                    pion_5[t_idx,x_idx,y_idx,z_idx] += contract(
                        "pwtzyx,wtzyxjiba,jk,wtzyxklba,li->pt",
                        momentum_phases,
                        prop1.data.conj(),
                        G5 @ G5,
                    ...
                    proton_45_p[t_idx,x_idx,y_idx,z_idx] += (contract("abc,def,gh,ij,kl,pwtzyx,wtzyxhjeb,wtzyxgida,wtzyxlkfc->pt",
                    ...
                    proton_45_m[t_idx,x_idx,y_idx,z_idx] += (contract("abc,def,gh,ij,kl,pwtzyx,wtzyxhjeb,wtzyxgida,wtzyxlkfc->pt",
                        eps_color,eps_color,charge @ G45, charge @ G45, parity_m,momentum_phases, prop2.data,prop2.data,prop2.data)
                    -
                    contract("abc,def,gh,ij,kl,pwtzyx,wtzyxhjeb,wtzyxgkdc,wtzyxlifa->pt",
                        eps_color,eps_color,charge @ G45, charge @ G45, parity_m,momentum_phases, prop2.data,prop2.data,prop2.data))

                    proton_5_p[t_idx,x_idx,y_idx,z_idx] += (contract("abc,def,gh,ij,kl,pwtzyx,wtzyxhjeb,wtzyxgida,wtzyxlkfc->pt",
                        eps_color,eps_color,charge @ G5, charge @ G5, parity_p,momentum_phases, prop2.data,prop2.data,prop2.data)
                    -
                    contract("abc,def,gh,ij,kl,pwtzyx,wtzyxhjeb,wtzyxgkdc,wtzyxlifa->pt",
                        eps_color,eps_color,charge @ G5, charge @ G5, parity_p,momentum_phases, prop2.data,prop2.data,prop2.data))

                    proton_5_m[t_idx,x_idx,y_idx,z_idx] += (contract("abc,def,gh,ij,kl,pwtzyx,wtzyxhjeb,wtzyxgida,wtzyxlkfc->pt",
    ...
    pion_45_np=core.gatherLattice(pion_45.get(),[5,-1,-1,-1])
    pion_5_np=core.gatherLattice(pion_5.get(),[5,-1,-1,-1])
    proton_45_p_np = core.gatherLattice(proton_45_p.get(), [5, -1, -1, -1])
    proton_45_m_np = core.gatherLattice(proton_45_m.get(), [5, -1, -1, -1])
    proton_5_p_np  = core.gatherLattice(proton_5_p.get(),  [5, -1, -1, -1])
    proton_5_m_np  = core.gatherLattice(proton_5_m.get(),  [5, -1, -1, -1])
    ...
    #ROLL every source so that time index 0 is t_src:  rolled[..., tau] = C[..., (t_src + tau) % 96]
        for t_idx, t0 in enumerate(t_src[i_cfg]):
            pion_45_np[t_idx]     = np.roll(pion_45_np[t_idx],     -t0, axis=-1)
            pion_5_np[t_idx]      = np.roll(pion_5_np[t_idx],      -t0, axis=-1)
            proton_45_p_np[t_idx] = np.roll(proton_45_p_np[t_idx], -t0, axis=-1)
            proton_45_m_np[t_idx] = np.roll(proton_45_m_np[t_idx], -t0, axis=-1)
            proton_5_p_np[t_idx]  = np.roll(proton_5_p_np[t_idx],  -t0, axis=-1)
            proton_5_m_np[t_idx]  = np.roll(proton_5_m_np[t_idx],  -t0, axis=-1)
            ...
            proton_45_p_np[t_idx][..., Lt - t0:] *= -1
            proton_45_m_np[t_idx][..., Lt - t0:] *= -1
            proton_5_p_np[t_idx][..., Lt - t0:]  *= -1
            proton_5_m_np[t_idx][..., Lt - t0:]  *= -1
        ...
        pion45_filename = f"pion_N{smear_steps}_rho{rho}_frac{smear_mom_x_str}_G45_cfg{cfg}.h5"
        pion5_filename = f"pion_N{smear_steps}_rho{rho}_frac{smear_mom_x_str}_G5_cfg{cfg}.h5"
        proton45_filename = f"proton_N{smear_steps}_rho{rho}_frac{smear_mom_x_str}_G45_cfg{cfg}.h5"
        proton5_filename  = f"proton_N{smear_steps}_rho{rho}_frac{smear_mom_x_str}_G5_cfg{cfg}.h5"
        ...
        with h5py.File(f"{g45_dir}/{pion45_filename}", "w") as f:
                ...
                dset.attrs["dim_spec"] = np.array(["t_src_list","x_src_list","y_src_list","z_src_list","momentum_list", "time"], dtype=h5py.string_dtype())
