...
# momentum transfers for the source phase (the same convention as fermilab_2pt_package.py and the local 3pt build; readers must
# match q rows by value through the q_list dataset, the order differs from the packager's): for every t_src the 8 spatial sources
# are averaged with the weight exp(-i 2pi/Ls q.x_src) / 8, so q index 0 = [0,0,0] is the plain mean.
# The files keep t_src but no longer the x, y, z source indices.
q_list = np.array([[0, 0, 0], [2, 0, 0], [-2, 0, 0], [0, 2, 0], [0, -2, 0], [1, 0, 0], [-1, 0, 0],
                   [0, 1, 0], [0, -1, 0], [1, -1, 0], [-1, 1, 0], [2, -2, 0], [-2, 2, 0]], dtype=np.int64)   # 13 q
src_phase_sign = -1
