...
parser.add_argument("--n", type=int, default=1)         # number of configs to measure: 1 per node on Frontier (2 h limit below 92 nodes)
args = parser.parse_args()
icfg0 = args.icfg
Ls = 32
Lt = 96
n = args.n  #number of configs to measure
mom_min = 0
mom_max = 6

# smearing: boosted Gaussian in Coulomb gauge (no gauge links)
...
q_list = np.array([[0, 0, 0], [2, 0, 0], [-2, 0, 0], [0, 2, 0], [0, -2, 0], [1, 0, 0], [-1, 0, 0],
                   [0, 1, 0], [0, -1, 0], [1, -1, 0], [-1, 1, 0], [2, -2, 0], [-2, 2, 0]], dtype=np.int64)   # 13 q's for now
src_phase_sign = -1
n_spatial_src = len(x_base) * len(y_base) * len(z_base)          # 8 spatial sources per t_src
run_parameters = {
    "Ls": Ls,
    "Lt": Lt,
    "cfgs_to_meas": n,
...
core.init([1, 1, 1, 8], resource_path="/lustre/orion/lgt132/scratch/sicheng/GLUON_ANALYSIS_MANUAL/2pt_production/.cache/quda")   # 8 GCDs, space unsplit for the FFT smearing
latt_info = core.LatticeInfo([Ls, Ls, Ls, Lt], -1, 1.0)
if latt_info.mpi_rank == 0:
    print(run_parameters)
print(f"rank {latt_info.mpi_rank}: HIP_VISIBLE_DEVICES={os.environ.get('HIP_VISIBLE_DEVICES')} cores {sorted(os.sched_getaffinity(0))}", flush=True)
dirac = core.getDirac(latt_info, -0.05138, 1e-10, 1000, 1.0, 1.04243, 1.04243, [[4, 4, 4, 3],[2,2,2,2]])   # local 32^3 x 12: coarse levels 8^3 x 4 and 4^3 x 2
G4 = gamma.gamma(8)
G5 = gamma.gamma(15)
G45 = gamma.gamma(7)
gamma_list = [G5, G45]
...
current_dir = "/lustre/orion/lgt132/scratch/sicheng/GLUON_ANALYSIS_MANUAL/2pt_production"
n_shape, n_frac, n_gamma = len(shape_list), len(mom_frac_list), len(gamma_list)
pion_gamma         = cp.zeros((n_shape, n_shape, n_frac, n_frac, n_gamma, n_gamma, t_src.shape[1], len(q_list), len(momentum_list), latt_info.Lt), "<c16")          #(shape_sink, shape_source, frac_sink, frac_source, gamma_sink, gamma_source, t, q, p, Lt)
proton_gamma_dirac = cp.zeros((n_shape, n_shape, n_frac, n_frac, n_gamma, n_gamma, 4, 4, t_src.shape[1], len(q_list), len(momentum_list), latt_info.Lt), "<c16")   #(shape_sink, shape_source, frac_sink, frac_source, gamma_sink, gamma_source, dirac_sink, dirac_source, t, q, p, Lt)
...
for i_cfg, cfg in tqdm(enumerate(measurement_list),desc=f"Processing cfgs"):
    ...
    deviceSynchronize()
    s = perf_counter()
    gauge_file = f"/lustre/orion/lgt132/world-shared/DATA/MILC/a09m310/gauge/l3296f211b630m0074m037m440d.{cfg}"
                        ...
                        for i_sink_shape, i_sink_frac in smear_types:
                            ...
                            for i_gamma_sink, gamma_sink in enumerate(gamma_list):
                                ...
                                for i_gamma_source, gamma_source in enumerate(gamma_list):
                                    ...
                                    pion_gamma[i_sink_shape, i_src_shape, i_sink_frac, i_src_frac, i_gamma_sink, i_gamma_source, t_idx] += contract(
                                    "q,li,lipt->qpt", src_phase, gamma_source_bar @ G5, pion_open)                                   # (n_q, 81, Lt)

                                    proton_gamma_dirac[i_sink_shape, i_src_shape, i_sink_frac, i_src_frac, i_gamma_sink, i_gamma_source, :, :, t_idx] += contract(
                                    "q,ij,ijlkpt->lkqpt", src_phase, charge @ gamma_source, proton_open)                            # (4, 4, n_q, 81, Lt)
    ...
    #ROLL every source so that time index 0 is t_src:  rolled[..., tau] = C[..., (t_src + tau) % 96]
        ...
        pion_filename   = f"pion_{smear_tag}_GEVP_qsrc_cfg{cfg}.h5"
        proton_filename = f"proton_{smear_tag}_GEVP_DIRAC_qsrc_cfg{cfg}.h5"
        gevp_dir = f"{current_dir}/{smear_tag}_GEVP_qsrc_ez"
        ...
        source_note = ("spatial sources contracted: for every t_src, mean over the 8 positions x_src_list x y_src_list x z_src_list, weighted by "
                       "exp(-i 2pi/Ls q.x_src) with q from the q_list dataset; q index 0 = [0,0,0] is the plain mean. This phase is ALREADY APPLIED: "
                       "do not apply it again in the 3pt build")

        with h5py.File(f"{gevp_dir}/{pion_filename}", "w") as f:
            dset = f.create_dataset("pion_gamma", data=pion_gamma_np)          # (nshape, nshape, nfrac, nfrac, 2, 2, nt, nq, nmom, 96)
            ...
            dset.attrs["dim_spec"] = np.array(["shape_sink","shape_source","frac_sink","frac_source","gamma_sink","gamma_source","t_src_list","q_list","momentum_list", "time"], dtype=h5py.string_dtype())
        ...
        with h5py.File(f"{gevp_dir}/{proton_filename}", "w") as f:
            ...
            dset.attrs["shape_list"] = "rows of shape_list dataset: [rho_T, rho_z]; index order (sink, source)"
            dset.attrs["mom_frac_list"] = "quark boost k = frac * mom_max in units of 2pi/L along z; index order (sink, source)"
            dset.attrs["dim_spec"] = np.array(["shape_sink","shape_source","frac_sink","frac_source","gamma_sink","gamma_source","dirac_sink","dirac_source","t_src_list","q_list","momentum_list", "time"], dtype=h5py.string_dtype())
            dset.attrs["spatial_sources"] = source_note
            dset.attrs["smearing"] = smearing_note
            dset.attrs["gauge_fixing"] = gauge_fix_note
            dset.attrs["momentum_convention"] = momentum_note
