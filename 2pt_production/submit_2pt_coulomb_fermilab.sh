...
#SBATCH -o ./logs/pt2_coulomb_boosted_fermilab_%j.out
#SBATCH -e ./logs/pt2_coulomb_boosted_fermilab_%j.err

# 2pt test (pion + proton), Coulomb-gauge boosted smearing, 4 GPUs on one lq2 node, grid [1,1,1,4] in the python script
cd $SLURM_SUBMIT_DIR
mkdir -p logs
...
# run parameters: first configuration of the block (override with: ICFG=100 sbatch submit_2pt_coulomb_fermilab.sh)
ICFG=${ICFG:-0}
