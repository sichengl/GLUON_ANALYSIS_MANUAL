...
def ratio_model(x, p):
    """R = C3/C2 keeping the excited state in the denominator:

       [M00 + Ai e^-dEi.tau + Af e^-dEf.(T-tau) + Afi e^-dEi.tau-dEf.(T-tau)]
       / [1 + r e^-dEi.T],      r = c1/c0 from the two-point fit
    """
    ...
    num = (p["M00"][w] + Ai * gv.exp(-dEi * tau) + Af * gv.exp(-dEf * tsep_tau)
           + p["Afi"][w] * gv.exp(-dEi * tau - dEf * tsep_tau))
    den = 1.0 + p["r"][w] * gv.exp(-dEi * x["tsep"])
    return num / den


def make_prior(n_w, forward, gap_i=0.6,width_i=1,gap_f=0.6, width_f=1,
               r_center=0.0, r_width=1.0):
    ...
    """n_w : the number of z's
       ...
       r_center, r_width : c1/c0 from the two-point fit, and its error
    """
    ...
    prior["r"]   = gv.gvar([r_center] * n_w, [r_width*R_WIDTH_FACTOR] * n_w)
...
def fit_one_point(tgf, pf, q):
    ...
    with h5py.File(TWOPT_FIT, "r") as f:
        ...
        gap_f_jk = gf["dE_jk"][:, 1]
        C = gf["C_jk"][:]
        r_jk = C[:, 1] / C[:, 0]      # r = c1/c0, the denominator's excited state
        if forward:
            gap_i_jk = gap_f_jk
        else:
    ...
    print(f"  {label}: r = c1/c0 = {r_mean:.4f} +- {r_err:.4f} "
          f"(prior width {R_WIDTH_FACTOR * r_err:.4f}),  "
          f"r e^-dE.T = {r_mean * np.exp(-gap_f_mean * tsep_fit_list[0]):.4f} "
          f"at tsep {tsep_fit_list[0]} -> "
          f"{r_mean * np.exp(-gap_f_mean * tsep_fit_list[-1]):.4f} "
          f"at tsep {tsep_fit_list[-1]}", flush=True)
    ...
    central_fit = lsqfit.nonlinear_fit(
        ...
        maxit=MAXIT, prior=make_prior(n_w, forward, gap_i_mean, gap_i_err,
                                      gap_f_mean, gap_f_err, r_mean, r_err))
    ...
    names = ["M00", "Ai", "Afi", "dEi", "r"] + ([] if forward else ["Af", "dEf"])
    ...
    for i in range(n_jk):
        jackknife_fit = lsqfit.nonlinear_fit(
            ...
            prior=make_prior(n_w, forward, gap_i_jk[i], gap_i_err,
                             gap_f_jk[i], gap_f_err, r_jk[i], r_err))
        for n in names:
    ...
    with h5py.File(out, "w") as f:
        ...
        f.create_dataset("r_prior_center_jk", data=r_jk)
        ...
        f.attrs["r_prior_width"] = R_WIDTH_FACTOR * r_err
        f.attrs["r_prior"] = (f"2pt c1/c0 per sample, width "
                              f"{R_WIDTH_FACTOR} x its jackknife error")
        f.attrs["ratio_model"] = ("[M00 + Ai e^-dEi.tau + Af e^-dEf.(T-tau) "
                                  "+ Afi e^-dEi.tau-dEf.(T-tau)] "
                                  "/ [1 + r e^-dEi.T]")
...
def plot_fit(x, samples, params, chi2dof, q_value, tgf, pf, q):
    ...
    for icol, w in enumerate(ws):
        ...
        for itsep, tsep in enumerate(tsep_fit_list):
            ...
            curve = curve / (1.0 + rr * np.exp(-de * tsep))
